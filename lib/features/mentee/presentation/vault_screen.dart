...
class VaultScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          bool hasVaultAccess = userTier == 'Premium' || userTier == 'Enterprise';
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text("Session Notes Vault", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
                hasVaultAccess || uid != null 
                    ? _buildSessionNotesList(uid) 
                    : _buildLockedTierNotice("Upgrade to Premium or Enterprise to access offline session notes."),

                const SizedBox(height: 25),

                // 3. CLASS RECORDINGS VAULT SECTION (NEW)
                const Text("Class Recordings Vault", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
                hasVaultAccess || uid != null
                    ? _buildSessionRecordingsList(uid)
                    : _buildLockedTierNotice("Upgrade to Premium or Enterprise to access class video & audio recordings."),
      ...
      child: Row(
        ...
        children: [
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text("TIER: ${tier.toUpperCase()}", style: const TextStyle(color: Colors.greenAccent, fontSize: 10, fontWeight: FontWeight.bold)),
              const SizedBox(height: 5),
              const Text("TOTAL KNOWLEDGE RETAINED", style: TextStyle(color: Colors.white60, fontSize: 11, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Text(totalHours, style: const TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.bold)),
              const SizedBox(height: 5),
              const Text("Cloud & Offline Synchronized", style: TextStyle(color: Colors.white70, fontSize: 11)),
            ],
          ),
          const CircleAvatar(
            radius: 28,
            backgroundColor: Colors.white10,
            child: Icon(Icons.folder_special, color: Colors.greenAccent, size: 28),
          )
        ],
      ),
    );
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Text("No session notes recorded yet.", style: TextStyle(color: Colors.grey, fontSize: 13));
        }
  ...
  Widget _buildSessionRecordingsList(String? uid) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Text("No class recordings available yet.", style: TextStyle(color: Colors.grey, fontSize: 13));
        }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Text(message, style: const TextStyle(fontSize: 12, color: Colors.black87))),
        ],
      ),
    );
  }
